...
public class TagManagerTest {
    ...
    void add() throws MyException {
        ...
        daoFactoryMockedStatic.when(DaoFactory::tagDao).thenReturn(tagDaoSQLMock);
        when(DaoFactory.tagDao().getAll()).thenReturn(tags);
        Mockito.doCallRealMethod().when(tagManager).add(tag);
        MyException quoteException = Assertions.assertThrows(MyException.class, () -> {
            tagManager.add(tag);}, "Can't add tag with ID. ID is autogenerated");

        Assertions.assertEquals("Can't add tag with ID. ID is autogenerated", quoteException.getMessage());
        daoFactoryMockedStatic.verify(DaoFactory::tagDao);
        Mockito.verify(tagManager).add(tag);
